package Homework;

import java.util.Scanner;

public class homework4 {
//	有個一維陣列如下：
//	{29, 100, 39, 41, 50, 8, 66, 77, 95, 15} 
//	請寫出一隻程式能輸出此陣列所有元素的平均值與大於平均值的元素
//	(提示：陣列，length屬性)
	
	public static void main(String[] args) {
		int[] intArray = {29, 100, 39, 41, 50, 8, 66, 77, 95, 15};
		int num = 0;
		int i = 0;
		for (; i < intArray.length; i++) {
			num += intArray [i];
			}
	
		int aver = num / intArray.length;
				
		System.out.println(aver);
		System.out.print("大於平均值的有");
		
		for (i = 0; i < intArray.length; i++) {
			if(intArray[i] > aver) {
				System.out.println(intArray [i] + " ");
			}
			}
		
		System.out.println("========================");
	
	
//	請建立一個字串，經過程式執行後，輸入結果是反過來的
//	例如String s = “Hello World”，執行結果即為dlroW olleH 
//	(提示：String方法，陣列)
	 String s = "hello";
	 String s1 = "";
	 for(int a = s.length() - 1; a >= 0; a--) {
		 s1 += s.substring(a , a + 1);
		 }
	 System.out.println(s1);
	
	 System.out.println("========================");
	 
	
//	有個字串陣列如下 (八大行星)：
//	{“mercury”, “venus”, “earth”, “mars”, “jupiter”, “saturn”, “uranus”, “neptune”} 
//	請用程式計算出這陣列裡面共有多少個母音(a, e, i, o, u) 
//	(提示：字元比對，String方法)
	
	 String[] star = { "mercury", "venus", "earth", "mars", "jupiter", "saturn", "uranus", "neptune" };
	 int count = 0;
	 
	 for(int b = 0; b < star.length; b++) {
		 String arr = star[b];
	 
		 for(int c = 0; c < arr.length(); c++) {
			 String word = arr.substring(c , c + 1);
			 if (word.equals("a") || word.equals("e") || word.equals("i") || word.equals("o") || word.equals("u")) {
                 count++;
		 }
		 
		 }
	 }
	 System.out.println("共有" + count +"個");
	 
	 System.out.println("========================");
	 
	 
//	 阿文上班時忘了帶錢包,想要向同事借錢,和他交情比較好的同事共有 5 個,其員工編號與身上現金列表如下：
//	 請設計一個程式,可以讓小華輸入欲借的金額後,便會顯示哪些員工編號的同事 
//	 有錢可借他;並且統計有錢可借的總人數:例如輸入 1000 就顯示「有錢可借的員工編號: 25 19 27 共 3 人!」
//	 (提示：Scanner，二維陣列)
	 Scanner sc = new Scanner(System.in);
     System.out.println("請輸入預借金額");
     int w1 = sc.nextInt();
     int[] employeeM = {2500, 800, 500, 1000, 1200};
     int[] employeeN = {25, 32, 8, 19, 27};

     System.out.print("有錢可借的員工編號: ");
     int count1 = 0;
     for(int e = 0; e < employeeN.length; e++){
         if(employeeM[e] >= w1){
             System.out.print(employeeN[e] + " ") ;
             count1++;
         }

     }
     System.out.print("共"+ count1 +"人");
	 
	 
	 
	 
//	 請設計由鍵盤輸入三個整數，分別代表西元yyyy年，mm月，dd日，執行後會顯示是該年的第幾天
//	 例：輸入 1984 9 8 三個號碼後，程式會顯示「輸入的日期為該年第252天」
//	 (提示1：Scanner，陣列)
//	 (提示2：需將閏年條件加入) 
//	 (提示3：擋下錯誤輸入：例如月份輸入為2，則日期不該超過29)
	 
     Scanner sc1 = new Scanner(System.in);
     System.out.println("請輸入三個整數");
     int yearI = sc1.nextInt();
     int monthI = sc1.nextInt();
     int dayI = sc1.nextInt();

     if (monthI < 1 || monthI > 12 ) {
         System.out.println("輸入的日期不正確");
         return;
     }


     int[] Month = {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
     if (yearI % 4 == 0 && yearI % 100 != 0 || yearI % 400 == 0) {
         Month[2] = 29;
     }

     if (dayI < 1 || dayI > Month[monthI]) {
         System.out.println("輸入的日期不正確");
         return;
     }

     int dayCount = 0;
     for (int f = 1; f < monthI; f++) {
         dayCount += Month[f];
     }
     dayCount += dayI;
     System.out.println("輸入的日期為該年第" + dayCount + "天");
	 
//	 班上有8位同學，他們進行了6次考試結果如下：
//	 請算出每位同學考最高分的次數 
//	 (提示：二維陣列)
     
     int[][] scores = {
             {10, 35, 40, 100, 90, 85, 75, 70},
             {37, 75, 77, 89, 64, 75, 70, 95},
             {100, 70, 79, 90, 75, 70, 79, 90},
             {77, 95, 70, 89, 60, 75, 85, 89},
             {98, 70, 89, 90, 75, 90, 89, 90},
             {90, 80, 100, 75, 50, 20, 99, 75}
     };

     int[] studentCount = new int[scores[0].length];
     for (int exam = 0; exam < scores.length; exam++) {
         int maxS = scores[exam][0];
         for (int student = 0; student < scores[exam].length; student++) {
             if (scores[exam][student] > maxS) {
                 maxS = scores[exam][student];
             }

         }for(int student = 0; student < scores[exam].length; student++){
             if (scores[exam][student] == maxS) {
            	 studentCount[student]++;
             }
         }
     	}

      System.out.println("每位同學考最高分的次數");
     for(int student = 0; student < scores[0].length; student++){
         System.out.println("同學" + (student + 1) + "：" + studentCount[student] + "次");
     }
 }
}
